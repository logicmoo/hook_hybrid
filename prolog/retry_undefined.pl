...
uses_undefined_hook.
 ...
 :- meta_predicate uses_predicate(*,*,*,*,*,*).
 :- meta_predicate uses_predicate_setting(1,*,*,*,*,*).
...
uses_predicate(_,SM,M,F,A,Act):- module_property(M,class(What)),
   ru:retry_undefined_hook(class(What),Setting),!,
   uses_predicate_setting(Setting,SM,M,F,A,Act).
...
uses_predicate(Setting,SM,M,F,A,Act):- uses_predicate_setting(Setting,SM,M,F,A,Act),!.


uses_predicate_setting(E,_,_,_,_,Error):- E=error, !,Error=error.
uses_predicate_setting(fail,_,_,_,_,_):- !,fail.
uses_predicate_setting(break,_,_,_,_,error):- !,dumpST_dbreak.

uses_predicate_setting(_,_, M,F,A, Retry):-  retry_undefined(M,F,A),!,Retry=retry.

uses_predicate_setting(kb_shared,System, M, F,A, retry):- !,
   show_failure(uses_undefined_hook(M)),
   create_predicate_inheritance(M,F,A),
   nop(System:import(M:F/A)),!.

% uses_predicate_setting(true,_, M,F,A, Retry):-  retry_undefined(M,F,A),!,Retry=retry.

uses_predicate_setting(DEF,_, M, F,A, Retry):-  call(DEF, M:F/A),!,Retry=retry.
